...
palmFilePicker::palmFilePicker(QObject *parent)
    : QObject(parent)
//////    ,  QFileDialog(parent),
//////    , m_invocationHandle(0)
    , m_mode(palmFilePicker::Picker)
...
/////        qWarning() << "palmFilePicker: unable to create invocation:" << strerror(errno);
/////        return;
/////    }
/////    errorCode = navigator_invoke_invocation_set_target(m_invocationHandle, s_filePickerTarget);
    ...
    if (errorCode != BPS_SUCCESS) {
        cleanup();
        qWarning() << "palmFilePicker: unable to set target:" << strerror(errno);
        return;
    }
    ...
    if (errorCode != BPS_SUCCESS) {
        cleanup();
        qWarning() << "palmFilePicker: unable to set action:" << strerror(errno);
        return;
    }
    ...
    if (errorCode != BPS_SUCCESS) {
        cleanup();
        qWarning() << "palmFilePicker: unable to set mime type:" << strerror(errno);
        return;
    }
*****/
...
/////    if (errorCode != BPS_SUCCESS) {
/////        cleanup();
/////        qWarning() << "palmFilePicker: unable to set data:" << strerror(errno);
/////        return;
/////    }
...
void palmFilePicker::close()
{
////    navigator_card_close_child();
    cleanup();
}

bool palmFilePicker::nativeEventFilter(const QByteArray&, void *message, long*)
{
/****
...
void palmFilePicker::setMode(palmFilePicker::Mode mode)
{
    m_mode = mode;
}

void palmFilePicker::setDefaultSaveFileNames(const QStringList &fileNames)
{
    m_defaultSaveFileNames = fileNames;
}

void palmFilePicker::addDefaultSaveFileName(const QString &fileName)
{
    m_defaultSaveFileNames.append(fileName);
}

void palmFilePicker::setDirectories(const QStringList &directories)
{
    m_directories = directories;
}

void palmFilePicker::addDirectory(const QString &directory)
{
    m_directories.append(directory);
}

void palmFilePicker::setFilters(const QStringList &filters)
{
    m_filters = filters;
}

void palmFilePicker::setTitle(const QString &title)
{
    m_title = title;
}

palmFilePicker::Mode palmFilePicker::mode() const
{
    return m_mode;
}

QStringList palmFilePicker::defaultSaveFileNames() const
{
    return m_defaultSaveFileNames;
}

QStringList palmFilePicker::directories() const
{
    return m_directories;
}

QStringList palmFilePicker::filters() const
{
    return m_filters;
}

QStringList palmFilePicker::selectedFiles() const
{
    return m_selectedFiles;
}

QString palmFilePicker::title() const
{
    return m_title;
}

void palmFilePicker::cleanup()
{
/////    if (m_invocationHandle) {
...
void palmFilePicker::handleFilePickerResponse(const char *data)
{
...
QString palmFilePicker::filePickerType() const
{
    bool images = false;
    bool video = false;
    bool music = false;
...
QString palmFilePicker::modeToString(palmFilePicker::Mode mode) const
{
    switch (mode) {
    case Picker:
...
bool palmFilePicker::eventFilter(QObject *target, QEvent *event)
{
...
////        }
/////    }
    return true; //// QFileDialog::eventFilter(target, event);
}
